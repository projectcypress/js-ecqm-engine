...
module.exports = class Executor {
  ...
  constructor(connection) {
    ...
    this.patientSource = new PatientSource(connection);
    this.valueSetSource = new ValueSetSource(connection);
  }
  ...
  */
   ...
   */
  async execute(patientIds, measureIds, connection, options = null) {
    ...
    const valueSets = await this.valueSetSource.findValueSetsByMongoidForMeasures(measures);
    await executor.executeForEachMeasure(measures, valueSets, handler, connection, options);
    const result = await handler.finish(connection);
    return result;
  }

  executeForEachMeasure(measures, valueSetsByMongoid, handler, connection, options) {
