...
module.exports = class MongoDBMeasureSource {
  constructor(connection) {
    this.CQLMeasure = connection.model('Measure', MeasureSchema);
    this.measure = null;
  }
  ...
  async findMeasure(measureId, callback = null) {
    ...
    } else if (measureId && measureId.constructor.name === 'ObjectID') {
    ...
    return this.CQLMeasure.findById(measureIdObj, (err, mes) => {
      ...
      if (mes === null) return TypeError('measure not found');

      self.measure = self.CQLMeasure(mes.toObject());
