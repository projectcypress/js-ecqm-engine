...
module.exports = class CQLCalculator {
  ...
  calculate(measure, patientSource, valueSetsByOid, options) {
    ...
    if (observations) {
      observations.forEach(obs => {
        generatedELMJSON = this.constructor.generateELMJSONFunction(obs.function_name, obs.parameter);
    ...
    const resultsRaw = this.constructor.executeEngine(
      elm,
      patientSource,
      measureValueSets,
    ...
    Object.keys(resultsRaw.patientResults).forEach(patientId => {
      ...
      measure.populations.forEach(population => {
        [populationResults, episodeResults] = Array.from(
          this.createPopulationValues(measure, population, resultsRaw, patientId, observationDefs)
        );
  ...
  createPopulationValues(measure, population, results, patientId, observationDefs) {
    ...
    if (!measure.episode_of_care) {
      populationResults = this.createPatientPopulationValues(measure, population, results, patientId, observationDefs);
      populationResults = this.handlePopulationValues(populationResults);
    } else {
      // episode of care based measure
      // collect results per episode
      episodeResults = this.createEpisodePopulationValues(measure, population, results, patientId, observationDefs);
  ...
  handlePopulationValues(populationResults) {
    ...
    if (populationResultsHandled.STRAT != null && this.constructor.isValueZero('STRAT', populationResults)) {
      // Set all values to 0
      ...
      this.constructor.isValueZero('DENOM', populationResults) ||
      ...
      if ('NUMEX' in populationResults) {
        populationResultsHandled.NUMEX = 0;
      } else if (populationResults.MSRPOPLEX != null && !this.constructor.isValueZero('MSRPOPLEX', populationResults))
        if ('values' in populationResults) {
          populationResults.values = [];
        }
  ...
  createPatientPopulationValues(measure, population, results, patientId, observationDefs) {
    ...
    Measure.ALL_POPULATION_CODES.forEach(popCode => {
      ...
      if (cqlMap[popCode]) {
        ...
        const popIndex = ResultsHelper.getPopIndexFromPopName(measure, population, popCode);
