...
module.exports = class CQLCalculator {
   ...
   * @param {Hash} valueSetsByMongoid - all ValueSets relevant to the measure hashed by Mongo ID
   */
  calculate(measure, patientSource, valueSetsByMongoid, options) {
    ...
    if (options && options.effective_date != null) {
      ...
      end = this.constructor.getConvertedTime(measure.get('measure_period').high.value);
    }
  ...
  valueSetsForCodeService(valueSetMongoids, hqmfSetId) {
    ...
    if (!this.valueSetsByMongoidCached[hqmfSetId]) {
      ...
      valueSetMongoids.forEach((mongoid) => {
        const specificValueSet = this.valueSetsByMongoid.find(vs => vs.id === mongoid.toString());
