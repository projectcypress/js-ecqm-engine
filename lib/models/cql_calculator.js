...
module.exports = class CQLCalculator {
  ...
  static calculate(measure, patientSource, valueSetsByOid, options) {
    ...
    if (options && options.effective_date != null) {
      start = CQLCalculator.getConvertedTime(options.effective_date);
      if (options.effective_date_end != null) {
        end = CQLCalculator.getConvertedTime(options.effective_date_end);
      } else {
        end = CQLCalculator.getConvertedTimeEndOfYear(options.effective_date);
      }
    } else {
      start = CQLCalculator.getConvertedTime(measure.measure_period.low.value);
      end = CQLCalculator.getConvertedTime(measure.measure_period.high.value);
    }
    ...
    if (observations) {
      observations.forEach(obs => {
        generatedELMJSON = CQLCalculator.generateELMJSONFunction(obs.function_name, obs.parameter);
    ...
    const measureValueSets = CQLCalculator.valueSetsForCodeService(measure.value_set_oids);

    // Calculate results for each CQL statement
    const resultsRaw = CQLCalculator.executeEngine(
      elm,
      patientSource,
      measureValueSets,
    ...
    Object.keys(resultsRaw.patientResults).forEach(patientId => {
      ...
      for (const [populationIndex, population] of measure.populations.entries()) {
        [populationResults, episodeResults] = Array.from(CQLCalculator.createPopulationValues(measure, population, resultsRaw, patientId, observationDefs, populationIndex));
        if (populationResults) {
          ...
          if (episodeResults != null) {
            ...
            if (Object.keys(episodeResults).length > 0) {
               ...
               */
              var populationRelevance = ResultsHelper.populationRelevanceForAllEpisodes(episodeResults);
            } else {
              // Use the patient based relevance if there are no episodes. This will properly set IPP or STRAT to true.
              var populationRelevance = ResultsHelper.buildPopulationRelevanceMap(populationResults);
            }
          } else {
          ...
          var statementRelevance = ResultsHelper.buildStatementRelevanceMap(populationRelevance, measure, population, populationIndex);
  ...
  static createPopulationValues(measure, population, results, patientId, observationDefs, populationIndex) {
    ...
    if (!measure.episode_of_care) {
      populationResults = CQLCalculator.createPatientPopulationValues(measure, population, results, patientId, observationDefs, populationIndex);
      populationResults = CQLCalculator.handlePopulationValues(populationResults);
    } else {
      // episode of care based measure
      // collect results per episode
      episodeResults = CQLCalculator.createEpisodePopulationValues(measure, population, results, patientId, observationDefs, populationIndex);
  ...
  static handlePopulationValues(populationResults) {
    ...
    if (populationResultsHandled.STRAT != null && CQLCalculator.isValueZero('STRAT', populationResults)) {
      // Set all values to 0
    ...
    } else if (CQLCalculator.isValueZero('IPP', populationResults)) {
      Object.keys(populationResults).forEach(key => {
        if (key !== 'STRAT') {
          if (key === 'values') {
            populationResultsHandled.values = [];
      ...
      CQLCalculator.isValueZero('DENOM', populationResults) ||
      CQLCalculator.isValueZero('MSRPOPL', populationResults)
    ) {
      ...
      !CQLCalculator.isValueZero('DENEX', populationResults) &&
      ...
      if ('NUMEX' in populationResults) {
        populationResultsHandled.NUMEX = 0;
      } else if (populationResults.MSRPOPLEX != null && !CQLCalculator.isValueZero('MSRPOPLEX', populationResults))
        if ('values' in populationResults) {
          populationResults.values = [];
        }
    } else if (CQLCalculator.isValueZero('NUMER', populationResults)) {
      if ('NUMEX' in populationResults) {
        populationResultsHandled.NUMEX = 0;
      }
    } else if (!CQLCalculator.isValueZero('NUMER', populationResults)) {
  ...
  static createPatientPopulationValues(measure, population, results, patientId, observationDefs, populationIndex) {
    ...
    Measure.ALL_POPULATION_CODES.forEach(popCode => {
      ...
      if (cqlMap[popCode]) {
        ...
        const cqlPopulation = definedPops[populationIndex];
  ...
  static valueSetsForCodeService(valueSetOids = []) {
    const valueSets = {};
