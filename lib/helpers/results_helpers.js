...
module.exports = class ResultsHelpers {
  ...
  static prettyResult(result, indentLevel, keyIndent) {
    ...
    } else if (result.constructor.name === 'model') {
      const description = result.description ? `${result.description}\n` : '';
      const startTime = result.relevantPeriod && result.relevantPeriod.low ? `START: ${moment.utc(result.relevantPeriod.low).format('MM/DD/YYYY h:mm A')}\n` : '';
      const endTime = result.relevantPeriod && result.relevantPeriod.high ? `STOP: ${moment.utc(result.relevantPeriod.high).format('MM/DD/YYYY h:mm A')}\n` : '';
