...
module.exports = class CalculatorHelpers {
  ...
  static setValueSetVersionsToUndefined(elm) {
    for (let elm_library of Array.from(elm)) {
      if (elm_library['library']['valueSets'] != null) {
        for (let valueSet of Array.from(elm_library['library']['valueSets']['def'])) {
  ...
  static valueSetsForCodeService(valueSetsByOid, valueSetOids = []) {
    ...
    valueSetOids.forEach(oid => {
      var specificValueSet = valueSetsByOid[oid];
