...
module.exports = class CQLResultsHelpers {
  ...
  static buildStatementRelevanceMap(populationRelevance, measure, populationSet) {
    ...
    if (measure.get('calculate_sdes') && populationSet.get('supplemental_data_elements')) {
      for (let statement of Array.from(populationSet.get('supplemental_data_elements'))) {
        // Mark all Supplemental Data Elements as relevant
        this._markStatementRelevant(measure.get('cql_statement_dependencies'), statementRelevance, measure.get('main_cql_library'), statement, "TRUE");
      }
    }
    ...
    for (let population in populationRelevance) {
      ...
      if (population === 'values') {
        for (let observation of Array.from(measure.get('observations'))) {
          this._markStatementRelevant(measure.get('cql_statement_dependencies'), statementRelevance, measure.get('main_cql_library'), observation.function_name, relevance);
        }
      } else {
        ...
        this._markStatementRelevant(measure.get('cql_statement_dependencies'), statementRelevance, measure.get('main_cql_library'), relevantStatement, relevance);
      }
    }
   ...
   */
  static _markStatementRelevant(cql_statement_dependencies, statementRelevance, libraryName, statementName, relevant) {
    ...
    if ((statementRelevance[libraryName][statementName] === 'NA') || (statementRelevance[libraryName][statementName] === 'FALSE')) {
      ...
      return Array.from(cql_statement_dependencies[libraryName][statementName]).map((dependentStatement) =>
        this._markStatementRelevant(cql_statement_dependencies, statementRelevance, dependentStatement.library_name, dependentStatement.statement_name, relevant));
    }
  }
  ...
  static buildStatementAndClauseResults(measure, rawClauseResults, statementRelevance, doPretty) {
    if (doPretty == null) { doPretty = false; }
    ...
    for (let lib in object) {
      ...
      clauseResults[lib] = {};
      for (let statementName in statements) {
        const rawStatementResult = this._findResultForStatementClause(measure, lib, statementName, rawClauseResults);
        statementResults[lib][statementName] = { raw: rawStatementResult};
        const isSDE = CQLMeasureHelpers.isSupplementalDataElementStatement(measure.get('populations').first(), statementName);
        if ((!measure.get('calculate_sdes') && isSDE) || (statementRelevance[lib][statementName] === 'NA')) {
          statementResults[lib][statementName].final = 'NA';
          if (doPretty) { statementResults[lib][statementName].pretty = 'NA'; }
          ...
          if (doPretty) { statementResults[lib][statementName].pretty = 'UNHIT'; }
        } else {
          if (this._doesResultPass(rawStatementResult)) {
            statementResults[lib][statementName].final = 'TRUE';
            if (doPretty) { statementResults[lib][statementName].pretty = this.prettyResult(rawStatementResult); }
          } else {
            ...
            if (rawStatementResult instanceof Array && (rawStatementResult.length === 0)) {
              ...
              if (doPretty) { statementResults[lib][statementName].pretty = "FALSE ([])"; }
            } else if (CQLMeasureHelpers.isStatementFunction(measure, lib, statementName)) {
             if (doPretty) { statementResults[lib][statementName].pretty = "FUNCTION"; }
            } else {
              if (doPretty) { statementResults[lib][statementName].pretty = `FALSE (${rawStatementResult})`; }
            }
          }
        }
        ...
        for (let localId in localIds) {
          ...
          clauseResult.final = this._setFinalResults({
            statementRelevance,
            statementName,
            rawClauseResults,
            lib,
  ...
  static prettyResult(result, indentLevel, keyIndent) {
    ...
    if (result instanceof cql.DateTime) {
      ...
      if (result) { return JSON.stringify(result, null, 2); } else { return 'null'; }
    }
  }
   ...
   */
  static _setFinalResults(params) {
    ...
    if (params.clause.isUnsupported != null) {
      ...
      finalResult = 'UNHIT';
    } else if (this._doesResultPass(params.rawResult)) {
      finalResult = 'TRUE';
    }
    return finalResult;
  }
   ...
   */
  static _findResultForStatementClause(measure, libraryName, statementName, rawClauseResults) {
    let library = null;
    let statement = null;
