 ...
 */
module.exports = class ResultsHelper {
  /**
  ...
  static buildStatementRelevanceMap(populationRelevance, measure, populationSet) {
    ...
    if (measure['calculate_sdes'] && populationSet.get('supplemental_data_elements')) {
    ...
    for (let population in populationRelevance) {
      ...
      if (population === 'values') {
        ...
        const populationIndex = ResultsHelper.getPopIndexFromPopName(measure, population, '');
  ...
  static buildStatementAndClauseResults(measure, rawClauseResults, statementRelevance, doPretty) {
    ...
    for (let lib in object) {
      ...
      for (let statementName in statements) {
        ...
        const isSDE = CQLMeasureHelpers.isSupplementalDataElementStatement(
          measure.get('populations').first(),
          statementName
        );
