...
module.exports = class MeasureHelpers {
  ...
  static buildDefineToFullStatement(measure) {
    const ret = {};
    for (let lib in measure.get('elm_annotations')) {
      const lib_statements = {};
      for (let statement of Array.from(measure.get('elm_annotations')[lib].statements)) {
        lib_statements[statement.define_name] = this._parseAnnotationTree(statement.children);
      }
  ...
  static parseAnnotationTree(children) {
    ...
    if (children.text !== undefined) {
      ...
      for (child of Array.from(children.children)) {
        ret = ret + this._parseAnnotationTree(child);
      }
    } else {
  ...
  static findAllLocalIdsInStatementByName(measure, libraryName, statementName) {
    ...
    for (let alias of Array.from(emptyResultClauses)) {
  ...
  static findAllLocalIdsInStatement(
    ...
    for (let k in statement) {
      ...
      if (k === 'return') {
        ...
        if (alId != null) {
          const typeClauseId = parseInt(statement.asTypeSpecifier.localId) - 1;
    ...
    return (() => {
      ...
      for (let k in statement) {
        ...
        if (Array.isArray(v) || typeof v === 'object') {
          result.push(
            this.findAllLocalIdsInSort(v, libraryName, localIds, aliasMap, emptyResultClauses, rootStatement)
          );
        } else {
  ...
  /**
     ...
     */
  static getStatementRelevanceForPopulationSet(measure, populationSet) {
    // create a population relevance map where every population is true.
    const populationRelevance = {};
    for (let popCode of Array.from(Thorax.Models.Measure.allPopulationCodes)) {
